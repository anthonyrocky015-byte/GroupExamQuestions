...
def register():
	if request.method == 'POST':
		...
		# if (theOTP == mOTP):
		cur = mysql.connection.cursor()
		cur.execute('INSERT INTO users(username,name,email, password, role, confirmed) values(%s,%s,%s,%s,1,%s)',
					(dbUsername, dbName, dbEmail, dbPassword, dbRole))
		mysql.connection.commit()
		cur.close()
		session.clear()
		flash('Thanks for registering! You are successfully registered.', 'success')
		return redirect(url_for('login'))
...
def login():
	if request.method == 'POST':
		...
		cur = mysql.connection.cursor()
		results = cur.execute('SELECT * from users where email = %s' , [username])
		if results > 0:
			data = cur.fetchone()
			print("data>>>>>>>>", data)
...
def verifyEmail():
	if request.method == 'POST':
		...
		if(theOTP == mOTP):
			...
			cur.close()
			session.clear()
			return render_template('login.html',success="Thanks for +ing! You are sucessfully verified.")
		else:
			return render_template('register.html',error="OTP is incorrect.")
...
def changePassword():
	if request.method == "POST":
		...
		cur = mysql.connection.cursor()
		results = cur.execute("SELECT * from users where email = '" + session['email'] + "'")
		if results > 0:
			data = cur.fetchone()
			password = data['password']
...
class UploadForm(FlaskForm):
	...
	test_id = IntegerField('Test Name')
...
def create_test():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		with open('questions/' + filename) as csvfile:
			...
			cur.execute('INSERT INTO teachers (username, test_id, start, end, password, subject, topic, email) values(%s,%s,%s,%s,%s,%s,%s,%s)',
			(dict(session)['username'], test_id, start_date_time, end_date_time, password, subject, topic, dict(session)['email']))
			cur.connection.commit()
			cur.close()
...
def deltidlist():
	cur = mysql.connection.cursor()
	results = cur.execute('SELECT test_id from teachers where email = %s', [session['email']])
	# if results > 0:
...
def test(testid):
	...
	if request.method == 'POST':
		...
		cur.close()
		completed=1
		for sa in range(1,int(results1['COUNT(qid)'])+1):
			answerByStudent = request.form[str(sa)]
			from datetime import date
			today = date.today()
			print(">>>>>>>>>>>>",answerByStudent)
			cur = mysql.connection.cursor()
			results1 = cur.execute('SELECT ans from questions where test_id = %s AND qid = %s', [testid, sa])
			results_ans = cur.fetchone()
			result_value = 0;
			if answerByStudent == results_ans['ans']:
				result_value = 1
			cur = mysql.connection.cursor()
			cur.execute('INSERT INTO students (username, test_id, score, date_taken, qid, ans) VALUES (%s, %s, %s, %s, %s, %s)',
						(session['username'], testid, result_value, today, sa, answerByStudent))
			mysql.connection.commit()
...
def give_test():
	...
	if request.method == 'POST' and form.validate():
		...
		if results > 0:
			...
			email_results = cur.execute('SELECT * from users where id = %s and email=%s', [user_id, email])
			if email_results > 0:
				start = data['start']
				start = str(start)
				end = data['end']
				...
				if password == password_candidate:
					...
					if datetime.strptime(start,"%Y-%m-%d %H:%M:%S") < now and datetime.strptime(end,"%Y-%m-%d %H:%M:%S") > now:
						results = cur.execute('SELECT completed from studentTestInfo where username = %s and test_id = %s', (session['email'], test_id))
						if results > 0:
...
def student_results(username, testid):
	if username == session['email']:
		...
		results = cur.execute('''
		    SELECT users.name AS name, users.email AS email, users.username AS username, test_id
		    FROM studentTestInfo, users
		    WHERE test_id = %s 
		      AND completed = 1 
		      AND studentTestInfo.username = users.email 
...
def studentRegister():
	if request.method == 'POST':
		...
		# if (theOTP == mOTP):
		cur = mysql.connection.cursor()
		cur.execute('INSERT INTO users(username,name,email, password, role, created_at, confirmed) values(%s,%s,%s,%s,%s,1)',
					(username, name, email, cpassword,role, session['user_id']))
		mysql.connection.commit()
		cur.close()
		flash('Thanks for Student Registering! You are successfully registered.', 'success')
...
def displaystudent():
	# if request.method == 'POST':
	tidoption = session['user_id']
	cur = mysql.connection.cursor()
	cur.execute('SELECT * from users where role =%s and created_at= %s', ['user', tidoption])
	callresults = cur.fetchall()
	cur.close()
	return render_template("student_register.html", callresults = callresults)
...
def displayadmin():
	...
	callresults = cur.fetchall()
	cur.close()
	return render_template("admin_register.html", callresults = callresults)
...
def edit_student(id):
    ...
    if request.method == 'POST':
        ...
        name = request.form['name']
        email = request.form['email']
        password = request.form['password']  # Assume plaintext for now; use hashing in production
        ...
        # Load student data to prefill the form
...
@app.route('/delete-student/<int:id>', methods=['GET'])
@is_logged
def delete_student(id):
    cur = mysql.connection.cursor()

    # Optional: check if the student exists first
    cur.execute('SELECT * FROM users WHERE id = %s', (id,))
    student = cur.fetchone()
